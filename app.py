...
# --- 1. 六都完整行政區資料庫 (全面導入衛福部真實醫療分級數據) ---
@st.cache_data
def load_liudu_data_v5_2():
    # 格式: (行政區, 面積, 緯度, 經度, 型態, 火車, 高鐵, 交流道, 國內機場, 國際機場, 醫學中心, 區域醫院, 地區醫院, 診所, 藥局)
    raw_cities = {
        "臺北市": [
            ("大安區", 11.36, 25.026, 121.543, "core", 0, 0, 1, 0, 0, 1, 2, 2, 452, 128),    # 國泰、仁愛聯醫
            ("信義區", 11.20, 25.033, 121.564, "core", 0, 0, 1, 0, 0, 1, 0, 1, 215, 78),     # 臺北醫學大學附設醫院
            ("中正區", 7.60, 25.032, 121.518, "core", 1, 0, 0, 0, 0, 2, 1, 1, 280, 85),     # 臺大醫院、台大兒童醫院、婦幼聯醫
            ("中山區", 13.68, 25.068, 121.533, "core", 0, 0, 3, 0, 0, 1, 1, 2, 410, 115),    # 馬偕紀念醫院
            ("萬華區", 8.85, 25.029, 121.499, "core", 1, 0, 2, 0, 0, 0, 1, 2, 165, 62),     # 昆明/西門聯醫、西園醫院
            ("松山區", 9.28, 25.060, 121.560, "core", 1, 0, 1, 1, 1, 0, 2, 2, 295, 90),     # 台北長庚、台安醫院
            ("大同區", 5.68, 25.063, 121.513, "core", 1, 0, 2, 0, 0, 0, 1, 1, 140, 55),     # 中興聯醫
            ("內湖區", 31.58, 25.069, 121.589, "suburb", 0, 0, 4, 0, 0, 1, 1, 1, 210, 82),   # 三軍總醫院
            ("南港區", 21.84, 25.055, 121.606, "suburb", 1, 1, 3, 0, 0, 0, 1, 1, 95, 41),
            ("文山區", 31.50, 24.998, 121.570, "suburb", 0, 0, 2, 0, 0, 0, 1, 2, 155, 68),    # 萬芳醫院(區域)
            ("士林區", 62.37, 25.093, 121.526, "suburb", 0, 0, 1, 0, 0, 1, 1, 1, 245, 92),    # 新光醫院
            ("北投區", 56.82, 25.132, 121.500, "suburb", 0, 0, 0, 0, 0, 2, 1, 2, 145, 59)     # 臺北榮總、振興醫院
        ],
        "新北市": [
            ("板橋區", 23.14, 25.011, 121.465, "core", 2, 1, 3, 0, 0, 1, 1, 4, 560, 185),    # 亞東醫院、新北聯醫板橋院區
            ("三重區", 16.32, 25.062, 121.498, "core", 0, 0, 3, 0, 0, 0, 1, 3, 310, 112),    # 新北聯醫三重院區
            ("中和區", 20.14, 24.999, 121.498, "core", 0, 0, 2, 0, 0, 1, 0, 3, 340, 125),    # 雙和醫院
            ("永和區", 5.71, 25.008, 121.516, "core", 0, 0, 0, 0, 0, 0, 1, 2, 220, 88),     # 永和耕莘
            ("新莊區", 19.74, 25.036, 121.445, "core", 0, 0, 3, 0, 0, 0, 1, 3, 305, 108),    # 輔大醫院(區域級以上)
            ("新店區", 120.22, 24.968, 121.541, "suburb", 0, 0, 3, 0, 0, 0, 2, 2, 240, 95),   # 台北慈濟、耕莘總院
            ("土城區", 29.56, 24.973, 121.444, "suburb", 0, 0, 2, 0, 0, 0, 1, 1, 160, 64),    # 土城醫院(長庚體系)
            ("蘆洲區", 7.44, 25.084, 121.474, "core", 0, 0, 1, 0, 0, 0, 0, 2, 175, 58),
            ("汐止區", 71.24, 25.067, 121.659, "suburb", 3, 0, 4, 0, 0, 0, 1, 1, 125, 48),    # 汐止國泰
            ("樹林區", 33.12, 24.991, 121.420, "suburb", 2, 0, 1, 0, 0, 0, 0, 3, 110, 45),
            ("淡水區", 70.65, 25.170, 121.442, "suburb", 0, 0, 0, 0, 0, 0, 1, 1, 135, 52),    # 淡水馬偕
            ("三峽區", 191.15, 24.933, 121.371, "rural", 0, 0, 1, 0, 0, 0, 1, 0, 85, 32),     # 恩主公醫院
            ...
            ("林口區", 54.15, 25.077, 121.391, "suburb", 0, 0, 2, 0, 0, 0, 0, 1, 105, 38),    # 鄰近龜山長庚醫療圈
            ...
            ("金山區", 49.21, 25.221, 121.637, "rural", 0, 0, 0, 0, 0, 0, 0, 1, 14, 6),       # 台大金山分院
        ...
        "桃園市": [
            ("桃園區", 34.80, 24.995, 121.306, "core", 1, 0, 2, 0, 0, 0, 2, 3, 485, 142),    # 臺大桃園分院、敏盛經國總院
            ("中壢區", 76.52, 24.966, 121.224, "core", 2, 0, 3, 0, 0, 0, 2, 4, 460, 135),    # 天晟醫院、新國民醫院
            ("平鎮區", 31.28, 24.945, 121.218, "suburb", 0, 0, 2, 0, 0, 0, 1, 1, 165, 58),    # 聯新國際醫院
            ("八德區", 33.71, 24.963, 121.294, "suburb", 0, 0, 1, 0, 0, 0, 0, 1, 130, 46),
            ("楊梅區", 89.12, 24.908, 121.146, "suburb", 4, 0, 3, 0, 0, 0, 1, 2, 115, 42),    # 怡仁綜合醫院
            ("蘆竹區", 75.50, 25.052, 121.288, "suburb", 0, 0, 3, 0, 0, 0, 0, 1, 120, 44),
            ("大園區", 87.39, 25.063, 121.215, "rural", 0, 0, 4, 0, 1, 0, 0, 1, 45, 18),
            ("龜山區", 72.01, 25.001, 121.338, "suburb", 0, 0, 1, 0, 0, 1, 0, 1, 95, 34),     # 林口長庚紀念醫院(全台最大醫學中心)
            ("大溪區", 105.12, 24.880, 121.287, "rural", 0, 0, 2, 0, 0, 0, 0, 1, 48, 19),
            ("龍潭區", 75.23, 24.862, 121.216, "suburb", 0, 0, 2, 0, 0, 0, 1, 1, 78, 29),     # 國軍桃園總醫院
            ("新屋區", 85.02, 24.972, 121.105, "rural", 0, 0, 1, 0, 0, 0, 1, 0, 22, 10),     # 桃園醫院新屋分院
            ("觀音區", 87.12, 25.036, 121.077, "rural", 0, 0, 2, 0, 0, 0, 0, 1, 32, 13),
            ("復興區", 350.78, 24.821, 121.352, "rural", 0, 0, 0, 0, 0, 0, 0, 0, 6, 2)
        ],
        "臺中市": [
            ("西屯區", 39.85, 24.182, 120.623, "core", 0, 0, 4, 0, 0, 1, 1, 2, 310, 98),     # 台中榮總、澄清中港院區
            ("北屯區", 62.70, 24.181, 120.697, "suburb", 1, 0, 4, 0, 0, 0, 1, 2, 285, 92),    # 台中慈濟醫院
            ("南屯區", 31.26, 24.137, 120.640, "core", 0, 0, 3, 0, 0, 0, 1, 1, 180, 58),     # 林新醫院
            ("北區", 6.93, 24.156, 120.682, "core", 0, 0, 0, 0, 0, 1, 1, 1, 265, 84),      # 中國醫藥大學附設醫院
            ("西區", 5.70, 24.141, 120.667, "core", 0, 0, 0, 0, 0, 0, 0, 2, 210, 72),
            ("東區", 9.28, 24.136, 120.693, "core", 1, 0, 0, 0, 0, 0, 1, 1, 115, 41),     # 台新醫院
            ("南區", 6.81, 24.118, 120.662, "core", 1, 0, 0, 0, 0, 0, 1, 1, 145, 49),     # 鄰近中山醫學大學
            ("中區", 0.88, 24.143, 120.683, "core", 1, 0, 0, 0, 0, 0, 1, 1, 65, 28),
            ("太平區", 120.75, 24.127, 120.722, "suburb", 0, 0, 2, 0, 0, 0, 1, 1, 170, 56),    # 國軍臺中總醫院
            ("大里區", 32.45, 24.102, 120.677, "suburb", 0, 0, 3, 0, 0, 0, 1, 3, 230, 78),    # 仁愛醫院大里院區
            ("霧峰區", 98.08, 24.062, 120.700, "rural", 0, 0, 4, 0, 0, 0, 1, 1, 55, 22),     # 亞洲大學附屬醫院
            ("烏日區", 43.40, 24.105, 120.623, "suburb", 2, 1, 4, 0, 0, 0, 1, 0, 68, 25),     # 林新醫院烏日院區
            ("豐原區", 41.18, 24.252, 120.720, "core", 1, 0, 2, 0, 0, 0, 1, 3, 185, 64),     # 部立豐原醫院
            ("后里區", 35.94, 24.305, 120.716, "rural", 2, 0, 1, 0, 0, 0, 0, 1, 42, 16),
            ("石岡區", 18.21, 24.275, 120.778, "rural", 0, 0, 0, 0, 0, 0, 0, 0, 12, 6),
            ("東勢區", 117.41, 24.258, 120.829, "rural", 0, 0, 0, 0, 0, 0, 0, 2, 45, 19),    # 東勢農民醫院
            ...
            ("沙鹿區", 40.46, 24.233, 120.565, "suburb", 1, 0, 2, 1, 0, 0, 2, 1, 140, 51),    # 童綜合醫院、光田綜合醫院
            ("龍井區", 38.04, 24.215, 120.547, "rural", 1, 0, 2, 0, 0, 0, 0, 0, 68, 24),
            ("梧棲區", 16.60, 24.255, 120.531, "rural", 0, 0, 1, 0, 0, 0, 1, 0, 45, 18),     # 童綜合梧棲院區
            ("清水區", 64.17, 24.269, 120.537, "rural", 1, 0, 3, 0, 0, 0, 0, 0, 75, 29),
            ("大甲區", 58.51, 24.348, 120.624, "suburb", 2, 0, 0, 0, 0, 0, 1, 1, 78, 31),     # 光田醫院大甲院區
        ...
        "臺南市": [
            ...
            ("東區 ", 13.44, 22.984, 120.222, "core", 1, 0, 1, 0, 0, 1, 0, 2, 280, 89),      # 國立成功大學醫學院附設醫院
            ("北區 ", 10.43, 23.006, 120.210, "core", 0, 0, 0, 0, 0, 0, 1, 2, 185, 62),      # 部立臺南醫院
            ("南區 ", 27.26, 22.960, 120.184, "core", 0, 0, 2, 1, 0, 0, 1, 1, 110, 39),      # 市立醫院(區域級，鄰近東/南區交界)
            ("中西區", 6.26, 22.992, 120.199, "core", 0, 0, 0, 0, 0, 0, 1, 2, 150, 55),      # 郭綜合醫院
            ("安南區", 107.20, 23.047, 120.185, "suburb", 0, 0, 3, 0, 0, 0, 1, 1, 135, 46),   # 臺南市立安南醫院(中國醫藥大學委託經營)
            ("永康區", 40.05, 23.025, 120.254, "core", 3, 0, 3, 0, 0, 1, 0, 3, 295, 94),      # 奇美醫學中心
            ("新營區", 38.54, 23.307, 120.317, "suburb", 1, 0, 1, 0, 0, 0, 1, 2, 98, 36),      # 部立新營醫院
            ("鹽水區", 52.26, 23.320, 120.266, "rural", 0, 0, 1, 0, 0, 0, 0, 0, 21, 9),
            ("白河區", 126.40, 23.351, 120.416, "rural", 0, 0, 1, 0, 0, 0, 0, 1, 24, 11),
            ("柳營區", 61.20, 23.275, 120.332, "rural", 2, 0, 1, 0, 0, 0, 1, 0, 18, 8),       # 柳營奇美醫院(區域醫院)
            ("後壁區", 72.22, 23.366, 120.362, "rural", 1, 0, 0, 0, 0, 0, 0, 0, 12, 6),
            ("東山區", 124.91, 23.325, 120.404, "rural", 0, 0, 1, 0, 0, 0, 0, 0, 11, 5),
            ("麻豆區", 53.97, 23.181, 120.249, "suburb", 0, 0, 2, 0, 0, 0, 1, 1, 62, 24),      # 麻豆新樓醫院
            ...
            ("佳里區", 38.94, 23.165, 120.177, "suburb", 0, 0, 0, 0, 0, 0, 1, 1, 72, 26),      # 佳里奇美醫院
            ...
            ("歸仁區", 55.79, 22.954, 120.294, "suburb", 0, 1, 2, 0, 0, 0, 0, 1, 76, 25),     # 鄰近交大/成大沙崙醫療圈
            ("關廟區", 53.64, 22.962, 120.328, "rural", 0, 0, 2, 0, 0, 0, 0, 0, 31, 13),
            ("龍崎區", 64.08, 22.966, 120.361, "rural", 0, 0, 1, 0, 0, 0, 0, 0, 2, 1)
        ],
        "高雄市": [
            ("三民區", 19.78, 22.643, 120.328, "core", 1, 0, 1, 0, 0, 1, 1, 4, 326, 89),      # 高雄醫學大學附設中和紀念醫院、健新/馨蕙馨等
            ("苓雅區", 8.15, 22.621, 120.329, "core", 0, 0, 2, 0, 0, 0, 2, 2, 245, 76),      # 阮綜合醫院、邱外科醫院
            ("左營區", 19.38, 22.690, 120.301, "core", 1, 1, 3, 0, 0, 1, 0, 2, 210, 68),      # 高雄榮民總醫院
            ("新興區", 1.97, 22.627, 120.304, "core", 0, 0, 0, 0, 0, 0, 0, 3, 115, 42),
            ("前金區", 1.85, 22.627, 120.296, "core", 0, 0, 0, 0, 0, 0, 1, 1, 88, 33),       # 市立大同醫院(區域)
            ("鹽埕區", 1.41, 22.625, 120.284, "core", 0, 0, 0, 0, 0, 0, 0, 1, 45, 19),
            ("鼓山區", 14.74, 22.639, 120.275, "core", 1, 0, 0, 0, 0, 0, 1, 1, 135, 46),      # 市立聯合醫院(區域)
            ("前鎮區", 19.12, 22.597, 120.322, "core", 0, 0, 3, 0, 0, 0, 0, 3, 195, 61),
            ("小港區", 45.64, 22.565, 120.338, "suburb", 0, 0, 1, 1, 1, 0, 1, 1, 110, 39),    # 市立小港醫院(高醫團隊經營，區域級)
            ("鳳山區", 26.75, 22.626, 120.359, "core", 1, 0, 1, 0, 0, 0, 1, 5, 340, 105),     # 市立鳳山醫院、大東醫院
            ("鳥松區", 24.59, 22.659, 120.364, "suburb", 0, 0, 0, 0, 0, 1, 0, 0, 42, 16),      # 高雄長庚紀念醫院(南台灣頂級醫學中心)
            ...
            ("岡山區", 47.94, 22.785, 120.295, "suburb", 1, 0, 3, 0, 0, 0, 1, 3, 125, 45),     # 秀傳高醫岡山醫院
            ("燕巢區", 65.39, 22.793, 120.362, "rural", 0, 0, 4, 0, 0, 0, 2, 0, 22, 10),      # 義大醫院、義大癌治療醫院(區域/準醫學中心級)
            ...
            ("旗山區", 94.61, 22.888, 120.482, "rural", 0, 0, 2, 0, 0, 0, 1, 1, 42, 19),       # 部立旗山醫院
            ...
            ("林園區", 32.29, 22.508, 120.396, "rural", 0, 0, 0, 0, 0, 0, 0, 2, 65, 24),       # 建佑醫院
